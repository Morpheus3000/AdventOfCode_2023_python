...
def main(file_path):
    ...
    seed_ranges = almanac['seeds'][1::2]
    for s_init, seed_range in tqdm(zip(seed_starts, seed_ranges),
                                   total=len(seed_starts)):
        for s in tqdm(range(s_init, s_init + seed_range), leave=False, total=seed_range):
