...
def expand_universe(universe, expansion_factor=1):
    ...
    for i, idx in enumerate(empty_row_idx):
        ...
        expanded_universe = np.insert(expanded_universe, in_idx, padding, axis=0)
    for i, idx in enumerate(empty_col_idx):
        in_idx = idx + (expansion_factor * i)
        expanded_universe = np.insert(expanded_universe, in_idx, padding, axis=1)
    # expanded_universe = np.insert(expanded_universe, empty_col_idx, padding, axis=1)
...
def main(file_path):
    ...
    expanded_universe = expand_universe(universe, expansion_factor=10)
    print('[I] Actual universe size: ', expanded_universe.shape)
    # Calculate the pairs
